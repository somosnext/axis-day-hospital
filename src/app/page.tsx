...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const description = settings.seo_description || "Pacotes de cirurgias eletivas para médicos externos em São Paulo, com foco em segurança, tecnologia e praticidade.";
  return {
    title,
    description,
...
export default async function Home() {
  ...
  const processSteps = Array.isArray(process?.content?.steps)
    ...
    : ["Entre em contato", "Informe sua necessidade", "Receba as informações do Axis", "Organize sua cirurgia"];
  const safeMapsEmbed = settings.maps_embed?.startsWith("https://www.google.com/maps/embed") ? settings.maps_embed : null;
  const jsonLd = {
            ...
            <Reveal>
              <div className="grid gap-14 lg:grid-cols-[.75fr_1.25fr]">
                ...
                <div>
                  ...
                  <p className="mt-8 max-w-3xl text-lg leading-8 text-ink/70">{positioning?.description || "O Axis disponibiliza pacotes de cirurgias eletivas para médicos externos que buscam um espaço para realizar seus procedimentos. Uma proposta orientada pelos pilares informados pela instituição."}</p>
                </div>
              </div>
            </Reveal>
        ...
        <section id="para-medicos" className="bg-navy text-white">
          <div className="grid lg:grid-cols-2">
            ...
            <div className="flex items-center px-6 py-20 sm:px-12 lg:px-[8vw]">
              <div>
                <p className="eyebrow text-white/60">{physicians?.eyebrow || "Para médicos"}</p>
                <h2 className="mt-6 text-section balance">{physicians?.title || "Você cuida da cirurgia. O Axis cuida da estrutura."}</h2>
                <p className="mt-7 max-w-xl leading-7 text-white/70">{physicians?.description || "O modelo é direcionado a médicos externos interessados em trazer seu movimento de cirurgias para o Axis Day Hospital. As condições específicas são apresentadas pelo contato comercial, sem promessas ou escopos não confirmados."}</p>
                <Button href={physicians?.cta_url || "#contato"} variant="light" className="mt-9">{physicians?.cta_label || "Quero conhecer as condições"}</Button>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="como-funciona" className="section-pad bg-white">
          <Container>
            <div className="grid gap-10 lg:grid-cols-2">
              ...
              <p className="max-w-xl self-end text-sm leading-7 text-ink/60">{process?.description || "Etapas iniciais administráveis no CMS. O fluxo definitivo deve ser validado pela operação antes da publicação."}</p>
            </div>
        ...
        <section className="relative min-h-[70svh] overflow-hidden bg-navy-950 text-white">
          ...
          <Container className="relative flex min-h-[70svh] items-center">
            <div className="max-w-4xl">
              ...
              <p className="mt-7 max-w-xl text-lg leading-8 text-white/70">{editorialCta?.description || "Fale com o Axis para conhecer as informações comerciais e avaliar a adequação à sua necessidade."}</p>
              <Button href={editorialCta?.cta_url || "#contato"} variant="light" className="mt-9">{editorialCta?.cta_label || "Quero falar com o Axis"}</Button>
            </div>
          </Container>
        </section>
        ...
        <section id="contato" className="section-pad border-t hairline">
          <Container>
            <div className="grid gap-16 lg:grid-cols-[.8fr_1.2fr]">
              <div>
                <Eyebrow>Contato comercial</Eyebrow><h2 className="mt-6 text-section text-navy">Traga sua próxima cirurgia para o Axis.</h2>
                <p className="mt-7 max-w-md leading-7 text-ink/65">Preencha os dados essenciais. CRM, estado e especialidade ajudam no atendimento, mas são opcionais nesta primeira conversa.</p>
